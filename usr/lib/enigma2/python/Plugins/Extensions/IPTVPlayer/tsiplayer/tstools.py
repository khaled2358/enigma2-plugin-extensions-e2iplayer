...
def resolve_liveFlash(link,referer):
	...
	Liste_films_data = re.findall('var hlsUrl =.*?\+.*?"(.*?)".*?enableVideo.*?"(.*?)"', data2, re.S)
	if Liste_films_data:
		tmp=Liste_films_data[0][1]
		tmp=tmp.replace(tmp[0]+tmp[1]+tmp[2]+tmp[3]+tmp[4]+tmp[5],tmp[0]+tmp[1]+tmp[2]+tmp[4]+tmp[5])
		url = Liste_films_data[0][0]+tmp	
